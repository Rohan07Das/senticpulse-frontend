'use client'; // Required for Next.js router
...
const GooeyNav: React.FC<GooeyNavProps> = ({
  items,
  activeIndex: externalActiveIndex, // 1. Rename to avoid collision
  animationTime = 600,
  particleCount = 15,
  ...
  // 2. Initialize with the external index
  const [activeIndex, setActiveIndex] = useState<number>(externalActiveIndex);

  // 3. THIS IS THE MAGIC FIX: Listen for URL/External changes and update the pill
  useEffect(() => {
  ...
  const makeParticles = (element: HTMLElement) => {
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      setTimeout(() => {
        ...
        // Particles now use the active pill color
        particle.style.setProperty('--color', `var(--nav-active-bg)`);
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLAnchorElement>, index: number, href: string) => {
    if (e.key === 'Enter' || e.key === ' ') {
      ...
      if (liEl) {
        handleClick({ currentTarget: liEl } as React.MouseEvent<HTMLAnchorElement>, index, href);
      }
    }
  };

  // This effect moves the pill whenever the activeIndex changes (either from click OR from the new URL effect)
  useEffect(() => {
    if (!navRef.current || !containerRef.current) return;
    
    // Safety check: Ensure the index exists in the DOM
  ...
  }, [activeIndex, items]); // Added items as dependency so it recounts if the nav changes between marketing/app
    ...
    <>
      <style>
        {`
          ...
          .effect.text {
            color: var(--nav-text); /* Dynamic text color */
            transition: color 0.3s ease;
          }
          .effect.text.active {
            color: var(--nav-active-text); /* Dynamic active text color */
          }
          .effect.filter {
            filter: url('#goo'); /* Uses the SVG filter instead of CSS blur/contrast */
          }
          .effect.filter::after {
            content: "";
            position: absolute;
            inset: 0;
            background: var(--nav-active-bg); /* Dynamic pill color */
            transform: scale(0);
            opacity: 0;
            z-index: -1;
          ...
          .point {
            background: var(--nav-active-bg) !important; /* Particles match the pill */
            opacity: 1;
      ...
      <div className="relative" ref={containerRef}>
        <nav className="flex relative" style={{ transform: 'translate3d(0,0,0.01px)' }}>
          <ul
            ...
            style={{ color: 'var(--nav-text)' }} /* Dynamic base text color */
          >
