...
export default function AccountHeader() {
    ...
    const fetchUser = async () => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          setUserData(data);
          // Sync local name just in case it changed in DB
          localStorage.setItem('userName', data.name);
        }
      } catch (err) {
        console.error("Neural Identity Link Interrupted");
        // Use local storage name ONLY as a temporary visual placeholder if API fails
        const localName = localStorage.getItem('userName');
        if (localName) {
          setUserData({ email: savedEmail, name: localName, tier: "Active" });
        }
      }
    };
  ...
  }, [pathname, API_URL]); // Added API_URL to dependency array
    ...
    <div className="fixed top-6 right-8 z-[100] flex items-center gap-4">
      <div className="hidden md:flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/70 dark:bg-black/40 backdrop-blur-md border border-slate-200 dark:border-white/10 shadow-sm">
        ...
        <span className="text-[9px] font-black text-slate-500 dark:text-[#b3ffe2] uppercase tracking-widest">
          Neural Link: {userData?.tier || 'Verified'}
        </span>
      </div>

      <div className="group relative">
        <button className="h-10 w-10 rounded-full bg-slate-900 dark:bg-[#b3ffe2] flex items-center justify-center text-white dark:text-black border-2 border-white dark:border-black shadow-lg hover:scale-110 transition-all">
          <User size={20} />
        </button>
